...
const RiktPage = ({ language }) => {
  const AgendaData = content[language]?.AgendaData || content["en"].AgendaData;
  ...
  // Agenda states
  const dates = AgendaData.map((item) => item.date);
  ...
  return (
    <section className="bg-[#185195] text-white font-sans">
      ...
      <div className="flex flex-col md:flex-row items-start justify-between px-4 sm:px-8 md:px-20 py-16 animate-fadeIn">
        ...
        <div className="flex flex-col justify-center items-center mt-6 md:mt-0 md:ml-12 max-w-xl text-justify text-base sm:text-lg md:text-2xl leading-relaxed">
          <p>
            ...
            <br /> <br />
            {riktData.registration.description1}
            <br />
            <b>{riktData.registration.description2}</b>
          </p>
          <a
      ...
      {/* Contact Section */}
      <div id="contact" className="w-full max-w-xl mx-auto text-center py-12">
      ...
      {/* Wavy Effect */}
      <div>
        <img src="/wavyeffect.png" alt="wavy design" className="w-full" />
      </div>
